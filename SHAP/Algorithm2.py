...
class TreeExplainer:
    ...
    def shap_values(self, X, **kwargs):
        ...
        self.tree_shap(self.tree, X, x_missing, phi)
        return phi
    ...
    def tree_shap(self, tree, x, x_missing, phi, condition = 0, condition_feature = 0):
        ...
        tree_shap_recursive(
            ...
            tree.threshold, tree.value, tree.weighted_n_node_samples,
            x, x_missing, phi, 0, 0, self.feature_indexes, self.zero_fractions, self.one_fractions, self.pweights,
            1, 1, -1, condition, condition_feature, 1
        )

def extend_path(feature_indexes, zero_fractions, one_fractions, pweights, unique_depth, zero_fraction, one_fraction, feature_index):
...
# 好像没有用处....
class Tree:
    def __init__(self, children_left, children_right, feature, threshold, value, node_sample_weight):
        ...
        self.max_depth = compute_expectations(self.children_left, self.children_right, self.node_sample_weight, self.values, 0)
...
def tree_shap_recursive(children_left, children_right, features, thresholds, values, node_sample_weight, 
                        x, x_missing, phi, node_index, unique_depth, parent_feature_indexes, parent_zero_fractions, parent_one_fractions, parent_pweights, parent_zero_fraction, parent_one_fraction, parent_feature_index, 
                        condition, condition_feature, condition_fraction):
    ...
    if condition == 0 or condition_feature != parent_feature_index:
        extend_path(feature_indexes, zero_fractions, one_fractions, pweights, unique_depth, parent_zero_fraction, parent_one_fraction, parent_feature_index)
    ...
    #internal node
        ...
        # 当处于NULL的时候，直接返回。。。。 接下来的分支探索全部取消dsd
        ...
        # divide up the condition_fraction among the recuresive calls
        ...
        tree_shap_recursive(children_left, children_right, features, thresholds, values, node_sample_weight, x, x_missing, phi, hot_index, unique_depth + 1, feature_indexes, zero_fractions, one_fractions, pweights, hot_zero_fraction * incoming_zero_fraction, incoming_one_fraction, split_index, condition, condition_feature, hot_condition_fraction)
        tree_shap_recursive(children_left, children_right, features, thresholds, values, node_sample_weight, x, x_missing, phi, cold_index, unique_depth +1, feature_indexes, zero_fractions, one_fractions, pweights, cold_zero_fraction * incoming_zero_fraction, 0, split_index, condition, condition_feature, cold_condition_fraction)


TreeExplainer(clf).shap_values(df)
